...
exports.getAllPosts = async (req, res) => {
  try {
    const posts = await Post.find().populate('userId', 'first_name last_name email'); 
...
exports.getPostById = async (req, res) => {
  try {
    const post = await Post.findById(req.params.id).populate('userId', 'first_name last_name email'); 
    if (!post) {
...
exports.editPost = async (req, res) => {
  try {
    ...
    if (post.userId.toString() !== userId.toString()) {
      return res.status(403).send({ status: 'Error', msg: 'Unauthorized to edit this post' });
    }
    ...
    res.status(500).send({ status: 'Error', msg: 'Failed to retrieve post for editing' });
  }
};
...
exports.updatePost = async (req, res) => {
  try {
    ...
    if (post.userId.toString() !== userId.toString()) {
      return res.status(403).send({ status: 'Error', msg: 'Unauthorized to update this post' });
    }
    ...
    res.status(200).send({ status: 'Success', msg: 'Post updated successfully', updatedPost });
  } catch (error) {
...
exports.deletePost = async (req, res) => {
  try {
    ...
    if (post.userId.toString() !== userId.toString()) {
      return res.status(403).send({ status: 'Error', msg: 'Unauthorized to delete this post' });
    }

    const deletedPost = await Post.findByIdAndDelete(postId);

    res.status(200).send({ status: 'Success', msg: 'Post deleted successfully', deletedPost });
  } catch (error) {
